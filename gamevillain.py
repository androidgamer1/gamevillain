...
client = commands.Bot(description="Here is some command for you", command_prefix=commands.when_mentioned_or("G!"), pm_help = True)
client.remove_command('help')


async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='for G!help'))
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Bot Name- Game Villain", color=0x00ff00)
    embed.set_footer(text="Game Villain")
...
async def on_reaction_add(reaction, user):
     if reaction.emoji == '🇬':           
            ...
            embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')
            embed.add_field(name = 'G!ping', value ='G!ping',inline = False)
     ...
     if reaction.emoji == '🇲':
           ...
           embed.set_image(url = 'https://image.ibb.co/caM2BK/help.gif')    
           embed.add_field(name =' G!setnick (admin is required)', value ='G!setnick @user (name)',inline = False)
           embed.add_field(name = 'G!purge (mod or admin is required)', value =' G!purge (number)',inline = False)
           embed.add_field(name = 'G!ban (mod or admin is required)', value ='G!ban @user',inline = False)
...
async def invite():
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(color = discord.Color((r << 16) + (g << 8) + b))
    embed.add_field(name = 'Link to invite me', value='https://discordapp.com/api/oauth2/authorize?client_id=518748797090791425&permissions=8&scope=bot')
